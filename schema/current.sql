...
CREATE TABLE stock_price_rollups_hourly (
  stock_id INTEGER NOT NULL,
  bucket_start INTEGER NOT NULL,
...
CREATE TABLE stock_price_rollups_daily (
  stock_id INTEGER NOT NULL,
  bucket_start INTEGER NOT NULL,
  open_price REAL NOT NULL,
  high_price REAL NOT NULL,
...
CREATE TABLE stock_ingestion_run_daily_stats (
...
CREATE INDEX idx_stock_price_rollups_hourly_bucket_stock
  ON stock_price_rollups_hourly(bucket_start ASC, stock_id ASC);

CREATE INDEX idx_stock_price_rollups_daily_bucket_stock
...
CREATE TABLE arrest_scout_results (
  ...
  fraud_delta INTEGER,
  current_criminaloffenses INTEGER,
  historical_criminaloffenses INTEGER,
  criminaloffenses_delta INTEGER,
  ...
  current_criminaloffenses_timestamp INTEGER,
  ...
  historical_criminaloffenses_timestamp INTEGER,
  lookback_seconds INTEGER NOT NULL,
...
CREATE TABLE arrest_scout_feedback (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_arrest_scout_feedback_result
  ON arrest_scout_feedback(result_id, created_at DESC);

CREATE INDEX idx_arrest_scout_feedback_created
  ON arrest_scout_feedback(created_at DESC);

CREATE INDEX idx_arrest_scout_feedback_target
...
CREATE TABLE chain_watch_check_ins (
  ...
  reminder_error TEXT,
  takeover_button_shown INTEGER NOT NULL DEFAULT 0,
