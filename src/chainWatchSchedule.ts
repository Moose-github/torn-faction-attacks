...
export async function readWatch(env: Env, id?: string | null): Promise<ChainWatchScheduleResponse> {
  ...
  const [sheets, slots, members] = await env.DB.batch([
    ...
    env.DB.prepare(`SELECT s.watch_id, s.sheet_id, s.start_at, s.assigned_to, s.cancelled, m.name AS member_name,
        c.confirmed_at AS check_in_confirmed_at, c.closed_at AS check_in_closed_at, c.end_at AS check_in_end_at
      FROM chain_watch_slots s LEFT JOIN home_faction_members m ON m.member_id = s.assigned_to
      LEFT JOIN chain_watch_check_ins c ON c.watch_id = s.watch_id AND c.start_at = s.start_at
        AND c.assignment_revision = s.check_in_revision AND c.assigned_to = s.assigned_to AND c.cancelled_at IS NULL
      WHERE s.watch_id = ? ORDER BY s.start_at`).bind(watch.id),
