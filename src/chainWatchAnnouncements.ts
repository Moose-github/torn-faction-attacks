...
type Kind = "intro" | "summary";
type Announcement = { payloads_json: string | null; message_ids_json: string; sent_at: number | null };

// Snapshot all pages before posting, then save each returned ID. A retry resumes
// the same summary after the last delivered page, even if assignments change.
async function deliverAnnouncement(env: Env, data: ChainWatchScheduleResponse, kind: Kind, now: number, stopAt: number): Promise<boolean> {
  ...
  try {
    ...
    const payloads: ReturnType<typeof announcementPayload>[] = row.payloads_json ? JSON.parse(row.payloads_json)
      : kind === "intro" ? [watchInfoPayload(env, data)] : watchSummaryPayloads(env, data);
...
export async function publishFinishedWatchSummaries(env: Env, now = nowSeconds(), stopAt = Date.now() + 15_000): Promise<void> {
  ...
  const due = await env.DB.prepare(`SELECT w.id FROM chain_watch_schedules w JOIN chain_watch_announcements a ON a.watch_id = w.id
    WHERE a.kind = 'summary' AND a.sent_at IS NULL AND a.sync_until <= ? AND w.guild_id = ?
  ...
  for (const row of due.results) {
    ...
    try {
      ...
      await deliverAnnouncement(env, data, "summary", now, stopAt);
